...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" version='1.1'>
  ...
  <xsl:template match="*[contains(@class,' topic/simpletable ')]">
  	<xsl:param name="current-language"><xsl:call-template name="get.current.language"/></xsl:param>
	  <fo:block>
  ...
  <xsl:template match="*[contains(@class,' topic/sthead ')]/*[contains(@class,' topic/stentry ')]" priority="2">
    <fo:table-cell start-indent="2pt" background-color="silver" padding="2pt" text-align="center" font-weight="bold">
			<xsl:call-template name="add.style">
				<xsl:with-param name="style" select="'frameall'"/>
				<xsl:with-param name="current-language"><xsl:call-template name="get.current.language"/></xsl:with-param>
			</xsl:call-template>    
      <xsl:attribute name="column-number"><xsl:number count="*"/></xsl:attribute>
