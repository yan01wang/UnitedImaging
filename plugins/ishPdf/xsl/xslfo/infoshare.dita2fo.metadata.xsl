...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:axf="http://www.antennahouse.com/names/XSL/Extensions" version="1.1">
  ...
  -->
	  ...
	  <xsl:variable name="summary.id">
	    <xsl:choose>
	      ...
	      <!--<xsl:value-of select="concat(ancestor-or-self::*/@xml:lang,'/',substring-after(ancestor-or-self::*/@oid,concat(ancestor-or-self::*/@xml:lang,'-')))"/>-->
	        <xsl:value-of select="concat(ancestor-or-self::*/@xml:lang,'/',substring-after(@oid,concat(ancestor-or-self::*/@xml:lang,'-')))"/>
	      </xsl:when>
	      <xsl:otherwise>
	      	<xsl:choose>
	      		<xsl:when test="@oid">
    ...
    <xsl:if test="translate($show-metadata,'YES','yes') = 'yes'">
      <xsl:message>Insert summary for summary.id [<xsl:value-of select="$summary.id" />] based upon oid [<xsl:value-of select="@oid"/>]</xsl:message>
