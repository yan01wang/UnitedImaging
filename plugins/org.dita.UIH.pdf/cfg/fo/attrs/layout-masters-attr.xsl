...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:attribute-set name="blank-simple-page-master">
    <xsl:attribute name="page-width">
      <xsl:value-of select="$page-width"/>
    </xsl:attribute>
    <xsl:attribute name="page-height">
      <xsl:value-of select="$page-height"/>
    </xsl:attribute> <!-- add space for the blank page region before -->
   <xsl:attribute name="margin-top">
      <xsl:value-of select="'0.95cm'"/>
    </xsl:attribute> 
  </xsl:attribute-set>
  ...
  <xsl:attribute-set name="region-body__index.odd" use-attribute-sets="region-body.odd">
	...
	<xsl:attribute name="column-gap">5pt</xsl:attribute>
  </xsl:attribute-set>
  ...
  <xsl:attribute-set name="blank-region-before">
     <xsl:attribute name="extent">1.56cm</xsl:attribute>
  </xsl:attribute-set>

  <xsl:attribute-set name="region-before">
    <xsl:attribute name="extent">
      <xsl:value-of select="'1.56cm'"/>
    </xsl:attribute>
  ...
  <xsl:attribute-set name="region-start">
    ...
    <xsl:attribute name="display-align">start</xsl:attribute>
  </xsl:attribute-set>
