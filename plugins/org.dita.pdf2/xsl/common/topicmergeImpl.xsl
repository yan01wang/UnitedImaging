...
-->
  ...
  <xsl:template match="dita-merge">
	...
	<xsl:param name="p_number" select="count(preceding-sibling::*[contains(@class,' bookmap/chapter ')])+1"/>
    <xsl:param name="level" select="count(ancestor-or-self::*[contains(@class,' map/topicref ')])"/>
    ...
    <xsl:template match="*[contains(@class,' topic/topic ')] | dita-merge/dita">
	  ...
	  <xsl:param name="p_dita_type" select="@xtrf"/>
      ...
      <xsl:variable name="idcount">
        ...
        <xsl:copy>
			...
			<!-- add chapter number for index -->
			<xsl:if test="not(contains($p_dita_type,'Librarytopic'))">
			    <xsl:attribute name="chapter_number">
