  ...
  <table cellpadding="4" cellspacing="0" summary="">
    ...
    <xsl:choose>
      ...
      <xsl:when test="@frame = 'sides'">
		...
		<xsl:with-param name="contents">border:0.5pt solid rgb(175.95,197.88,205.02);</xsl:with-param>
		</xsl:call-template>
      </xsl:when>
	  ...
	  <!-- 图注表格线无 则设置width 且td行距缩小 -->
        <xsl:attribute name="frame">void</xsl:attribute>
        <xsl:attribute name="border">1</xsl:attribute>
		<xsl:attribute name="class">figNote</xsl:attribute>
      </xsl:when>
      <xsl:otherwise>
...
<xsl:template name="doentry">
  ...
  <!-- 获取表头单元格属性 colsep=0显示竖白线 -->
	    <xsl:if test="@colsep = 0">
	     <xsl:text>border-right:0.5pt solid white;</xsl:text>
	    </xsl:if>
	    <!-- 获取表头单元格属性 rowsep=0显示横白线 -->
	   <xsl:if test="@rowsep = 0">
	     <xsl:text>border-bottom:0.5pt solid white;</xsl:text>
	   </xsl:if>
    </xsl:with-param>
  </xsl:call-template>
   ...
   <xsl:call-template name="setid"/>
    <xsl:choose>
     ...
     <xsl:otherwise>
	  ...
	  <!-- simpletable border=1 set border-color -->
	  <xsl:call-template name="style">
		<xsl:with-param name="contents">border:0;</xsl:with-param>
		</xsl:call-template>
     </xsl:otherwise>
    </xsl:choose>
    <xsl:call-template name="commonattributes"/>
...
<xsl:template name="topic.sthead_stentry">
  <th>
    <xsl:call-template name="style">
      <xsl:with-param name="contents">
        <xsl:text>vertical-align:middle;border-left:0.5pt solid white;</xsl:text>
        <xsl:call-template name="th-align"/>
      </xsl:with-param>
    </xsl:call-template>
