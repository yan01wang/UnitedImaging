...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="*[contains(@class, ' topic/related-links ')]" name="header_button">
  <xsl:param name="nextbtn" select="'Y'"/>
  <xsl:param name="prevbtn" select="'Y'"/>
    <xsl:for-each select="descendant-or-self::*[contains(@class, ' topic/related-links ')][*[@role = 'next'] or *[@role='previous']]">
      <xsl:value-of select="$newline"/>
	   <xsl:if test="$include.roles = 'parent'"> 
         <xsl:for-each select="*[@href][@role = 'previous']">
            <xsl:apply-templates select=".">
			<xsl:with-param name="prevbtn" select="$prevbtn"/>
			</xsl:apply-templates>
          </xsl:for-each>
        </xsl:if>
		 ...
		 <xsl:if test="$include.roles = 'next'">
          ...
          <xsl:for-each select="*[@href][@role = 'next']">
            <xsl:apply-templates select=".">
			<xsl:with-param name="nextbtn" select="$nextbtn"/>
			</xsl:apply-templates>
          </xsl:for-each>
        </xsl:if>
      <xsl:value-of select="$newline"/>
    ...
    <xsl:for-each select="descendant-or-self::*[contains(@class, ' topic/related-links ')][*[@role = 'ancestor'] or *[@role='parent']]">
      <xsl:value-of select="$newline"/>
...
Each child is indented, the linktext is bold, and the shortdesc appears in normal text directly below the link, to create a summary-like appearance.-->
  ...
  <xsl:param name="header_title"/>
   <xsl:variable name="chapterToc"
                  select="descendant-or-self::*[contains(@class, ' topic/linkpool ')][contains(@ChapterToc ,'bookmap/chapter')]"/>
   ...
   <xsl:variable name="children"
    ...
    <xsl:choose>
	<xsl:when test="$children and $chapterToc and $header_title != 'Y'">
	 <xsl:value-of select="$newline"/>
      <ul class="ullinks">
        <xsl:value-of select="$newline"/>
  ...
  <xsl:param name="header_title"/>
    <xsl:variable name="children"
                  select="descendant::*[contains(@class, ' topic/link ')]
                                       ...
                                       [not(ancestor::*[contains(@class, ' topic/linklist ')])]"/>
    <xsl:choose>
	<xsl:when test="$children and $header_title!='Y'">
	 <xsl:value-of select="$newline"/>
  ...
  <xsl:param name="header_title"/>
    <xsl:for-each select="descendant::*[contains(@class, ' topic/link ')]
      ...
      <xsl:value-of select="$newline"/>
      <span class="familylinks">
        <xsl:value-of select="$newline"/>

		<xsl:choose>
		<xsl:when test="$NOPARENTLINK = 'no' and $include.roles = 'parent' and $header_title = 'Y'">
		 <xsl:choose>
  ...
  <!--OUTPUT THE basic child processing-->
  <xsl:template match="*[contains(@class, ' topic/link ')][@role = ('child', 'descendant')]" priority="2" name="topic.link_child">
    <xsl:param name="link_level"  select="count(ancestor-or-self::*[contains(@class, ' topic/link ')])"/>
  ...
  <xsl:variable name="el-name">
    ...
    <xsl:element name="{$el-name}">
      ...
      <strong>
		...
		<!-- can control toc level during link_level -->
          <xsl:apply-templates select="." mode="add-linking-attributes"/>
		  
          <xsl:apply-templates select="." mode="add-hoverhelp-to-child-links"/>
		  <xsl:choose>
		  <xsl:when test="$link_level = 2">
		  ...
		  <xsl:text>margin-left:2em;</xsl:text>
		   ...
		   <xsl:when test="$link_level = 3">
		  <xsl:call-template name="style">
		  <xsl:with-param name="contents">
		  <xsl:text>margin-left:4em;</xsl:text>
		  </xsl:with-param>
		  </xsl:call-template>
		  </xsl:when>
		  <xsl:when test="$link_level = 4">
		   <xsl:call-template name="style">
		  <xsl:with-param name="contents">
		  <xsl:text>margin-left:6em;</xsl:text>
		  </xsl:with-param>
		  </xsl:call-template>
		  </xsl:when>
		  <xsl:otherwise>
  ...
  <xsl:param name="prevbtn"/>
    ...
    <!--  <xsl:apply-templates select="." mode="add-desc-as-hoverhelp"/>-->
      ...
      <!--use linktext as linktext if it exists, otherwise use href as linktext-->
      <xsl:choose>
	  <xsl:when test="$prevbtn = 'Y'">
	  <img border="0" src="{$PATH2PROJ}artwork/prev.gif"/>
	  </xsl:when>
	    <xsl:when test="$nextbtn = 'Y'">
	  <img border="0" src="{$PATH2PROJ}artwork/next.gif"/>
	  </xsl:when>
      <xsl:when test="*[contains(@class, ' topic/linktext ')] and $nextbtn != 'Y' and $prevbtn !='Y'">
          <xsl:apply-templates select="*[contains(@class, ' topic/linktext ')]"/>
      </xsl:when>
      <xsl:otherwise>
          <!--use href-->
