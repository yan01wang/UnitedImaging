...
<xsl:stylesheet version="2.0"
  ...
  <xsl:param name="variableUIHFiles">string.xml</xsl:param>
  ...
  <xsl:variable name="UIHstringFiles" select="document($variableUIHFiles)/langlist/lang" as="element(lang)*"/>
  ...
  <xsl:template name="getUIHVariable">
    <xsl:param name="id" as="xs:string"/>
    <xsl:param name="params" as="node()*"/>
    <xsl:sequence select="dita-ot:get-UIH-variable(., $id, $params)"/>
  </xsl:template>

  <xsl:template name="findUIHString">
    <xsl:param name="id" as="xs:string"/>
    <xsl:param name="params" as="node()*"/>
    ...
    <xsl:choose>
      <xsl:when test="exists($l)">
        <xsl:variable name="stringfile" select="$UIHstringFiles[@xml:lang = $l]/@filename" as="xs:string*"/>
        <xsl:variable name="str" as="element()*">
          <xsl:for-each select="$stringfile">
            <xsl:sequence select="document(., $UIHstringFiles[1])/*/*[@name = $id or @id = $id]"/><!-- strings/str/@name opentopic-vars:vars/opentopic-vars:variable/@id -->
          </xsl:for-each>
        </xsl:variable>
        <xsl:choose>
