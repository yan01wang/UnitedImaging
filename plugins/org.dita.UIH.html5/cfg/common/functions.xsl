...
<xsl:stylesheet version="2.0"
  ...
  <xsl:function name="dita-ot:get-UIH-current-language" as="xs:string">
  ...
  <xsl:function name="dita-ot:get-UIH-iso-language-code" as="xs:string">
    <xsl:param name="lang" as="xs:string"/>
    <xsl:sequence select="tokenize($lang, '-')[1]"/>
  </xsl:function>

  <xsl:function name="dita-ot:get-UIH-language-codes" as="xs:string*">
    <xsl:param name="lang" as="xs:string"/>
	<xsl:variable name="lan" select="translate($lang,'-','_')"/>
    <xsl:sequence select="$lan"/>
  </xsl:function>
  ...
  <xsl:function name="dita-ot:get-UIH-variable" as="node()*">
    <xsl:param name="ctx" as="node()"/>
    <xsl:param name="id" as="xs:string"/>
    <xsl:param name="params" as="node()*"/>

    <xsl:call-template name="findUIHString">
      <xsl:with-param name="ctx" select="$ctx" tunnel="yes"/>
      <xsl:with-param name="id" select="$id"/>
      <xsl:with-param name="params" select="$params"/>
      <xsl:with-param name="ancestorlang"
        select="dita-ot:get-UIH-language-codes(dita-ot:get-UIH-current-language($ctx))"/>
      <xsl:with-param name="defaultlang" select="dita-ot:get-UIH-language-codes($DEFAULTLANG)"/>
    </xsl:call-template>
  </xsl:function>
  ...
  <xsl:function name="dita-ot:get-UIH-variable" as="node()*">
    ...
    <xsl:sequence select="dita-ot:get-UIH-variable($ctx, $id, ())"/>
  </xsl:function>
