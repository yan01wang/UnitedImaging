...
<xsl:stylesheet version="2.0" 
  ...
  <xsl:param name="include.rellinks" select="'parent child sibling descendant other'"/>
  ...
  <xsl:template match="*[contains(@class, ' map/topicref ')]" mode="link-from">
    ...
    <xsl:if test="$include.roles = 'child'">
      <xsl:apply-templates select="." mode="link-to-children"/>  
	   <!-- <xsl:apply-templates mode="link_fromsss">
   ...
   <xsl:template match="*[contains(@class, ' map/topicref ')]/*[contains(@class, ' map/topicref ')]" mode="link_fromsss">
  ...
  <xsl:template match="*[contains(@class, ' map/topicref ')]
    ...
    <xsl:if test="not(@processing-role = 'resource-only') and
      ...
      <linkpool class="- topic/linkpool ">
		...
		<xsl:attribute name="ChapterToc" select="@class"/>
  ...
  <xsl:template mode="link" 
    ...
    <xsl:if test="not(($FINALOUTPUTTYPE = 'PDF' or $FINALOUTPUTTYPE = 'IDD') and @print = 'no') and 
      ...
      <link class="- topic/link ">
		...
		<xsl:if test="*[contains(@class, ' map/topicref ')] and $role = 'child'">
		    <xsl:apply-templates select="*[@href and not(@href = '')]
			[not(@linking = ('none', 'sourceonly'))][not(@processing-role = 'resource-only')]" 
			mode="link">
			<xsl:with-param name="role">child</xsl:with-param>
			</xsl:apply-templates>
		</xsl:if>
